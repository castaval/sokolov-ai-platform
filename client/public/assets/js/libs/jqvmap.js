...
var JQVMap = function (l) {
  ...
  for (var i in ((this.selectedRegions = []),
  (this.multiSelectRegion = l.multiSelectRegion),
  (this.container = l.container),
  (this.defaultWidth = a.width),
  (this.defaultHeight = a.height),
  (this.color = l.color),
  (this.selectedColor = l.selectedColor),
  (this.hoverColor = l.hoverColor),
  (this.hoverColors = l.hoverColors),
  (this.hoverOpacity = l.hoverOpacity),
  this.setBackgroundColor(l.backgroundColor),
  (this.width = l.container.width()),
  (this.height = l.container.height()),
  this.resize(),
  jQuery(window).resize(function () {
    ...
    if (a && i) {
      ...
      jQuery(l.container).trigger(n, [a, i]),
        t &&
          (jQuery('.jqvmap-pin').remove(),
          (e.pinHandlers = !1),
          e.placePins(t.pins, t.mode));
    }
  }),
  (this.canvas = new VectorCanvas(this.width, this.height, l)),
  l.container.append(this.canvas.canvas),
  this.makeDraggable(),
  ...
  (this.label = jQuery('<div/>')
    ...
    .hide()),
  l.enableZoom &&
    (jQuery('<div/>').addClass('jqvmap-zoomin').text('+').appendTo(l.container),
    jQuery('<div/>')
  ...
  (e.countries = []),
  a.paths)) {
    var n = this.canvas.createPath({ path: a.paths[i].path });
    n.setFill(this.color),
      (n.id = e.getCountryId(i)),
      (e.countries[i] = n),
      'svg' === this.canvas.mode
        ? n.setAttribute('class', 'jqvmap-region')
        : jQuery(n).addClass('jqvmap-region'),
      jQuery(this.rootGroup).append(n);
  }
  if (
    (jQuery(l.container).delegate(
      ...
      function (t) {
        ...
        (n = n.toLowerCase()),
          'mouseover' === t.type
            ? (jQuery(l.container).trigger(s, [n, a.paths[n].name]),
              s.isDefaultPrevented() || e.highlight(n, i),
              l.showTooltip &&
                (e.label.text(a.paths[n].name),
                jQuery(l.container).trigger(o, [e.label, n]),
            ...
            : (e.unhighlight(n, i),
              e.label.hide(),
              jQuery(l.container).trigger('regionMouseOut.jqvmap', [
                n,
                a.paths[n].name,
              ]));
      },
    ...
    jQuery(l.container).delegate(
      ...
      function (t) {
        ...
        if (
          ((n = n.toLowerCase()),
          jQuery(l.container).trigger(o, [n, a.paths[n].name]),
          !l.multiSelectRegion && !o.isDefaultPrevented())
        )
          for (var s in a.paths)
            (e.countries[s].currentFillColor =
              e.countries[s].getOriginalFill()),
              e.countries[s].setFill(e.countries[s].getOriginalFill());
        o.isDefaultPrevented() ||
          (e.isSelected(n) ? e.deselect(n, i) : e.select(n, i));
      },
    ),
    l.showTooltip &&
      l.container.mousemove(function (l) {
        if (e.label.is(':visible')) {
          var t = l.pageX - 15 - e.labelWidth,
            a = l.pageY - 15 - e.labelHeight;
          0 > t && (t = l.pageX + 15),
            0 > a && (a = l.pageY + 15),
            e.label.css({ left: t, top: a });
        }
      }),
    this.setColors(l.colors),
    ...
    l.values && ((this.values = l.values), this.setValues(l.values)),
    l.selectedRegions)
  )
    if (l.selectedRegions instanceof Array)
      for (var o in l.selectedRegions)
        this.select(l.selectedRegions[o].toLowerCase());
    else this.select(l.selectedRegions.toLowerCase());
  if (
    (this.bindZoomButtons(),
    l.pins &&
      ((t = { pins: l.pins, mode: l.pinMode }),
      (this.pinHandlers = !1),
      this.placePins(l.pins, l.pinMode)),
    l.showLabels)
  ) {
    this.pinHandlers = !1;
    var s = {};
    for (i in e.countries)
      'function' != typeof e.countries[i] &&
        ((l.pins && l.pins[i]) || (s[i] = i.toUpperCase()));
    (t = { pins: s, mode: 'content' }), this.placePins(s, 'content');
  }
  JQVMap.mapIndex++;
};
(JQVMap.prototype = {
    ...
    jQuery.fn.vectorMap = function (e) {
      ...
      if ('addMap' === e) JQVMap.maps[arguments[1]] = arguments[2];
        ...
        if ('set' !== e || !l[arguments[1]]) {
          if ('string' == typeof e && 'function' == typeof i[e])
            return i[e].apply(i, Array.prototype.slice.call(arguments, 1));
          for (var n in (jQuery.extend(a, e),
          (a.container = this),
          this.css({ position: 'relative', overflow: 'hidden' }),
          (i = new JQVMap(a)),
          this.data('mapObject', i),
          this.unbind('.jqvmap'),
          t))
            a[n] && this.bind(t[n] + '.jqvmap', a[n]);
        ...
        ].apply(i, Array.prototype.slice.call(arguments, 2));
      }
    };
  })(jQuery),
  ...
  (JQVMap.prototype.applyTransform = function () {
    ...
    this.defaultWidth * this.scale <= this.width
      ? ((l = (this.width - this.defaultWidth * this.scale) / (2 * this.scale)),
        (e = (this.width - this.defaultWidth * this.scale) / (2 * this.scale)))
      : ((l = 0),
        (e = (this.width - this.defaultWidth * this.scale) / this.scale)),
  ...
  (JQVMap.prototype.highlight = function (l, t) {
    (t = t || jQuery('#' + this.getCountryId(l))[0]),
      this.hoverOpacity
        ...
        ? ((t.currentFillColor = t.getFill() + ''),
          t.setFill(this.hoverColors[l]))
        : this.hoverColor &&
          ((t.currentFillColor = t.getFill() + ''), t.setFill(this.hoverColor));
  }),
    ...
    (L.isMoving = !1),
      ...
      this.container
        .mousemove(function (e) {
          return (
            h &&
              ((L.transX -= (l - e.pageX) / L.scale),
              (L.transY -= (t - e.pageY) / L.scale),
              L.applyTransform(),
              (l = e.pageX),
              (t = e.pageY),
              (L.isMoving = !0),
              L.isMovingTimeout && clearTimeout(L.isMovingTimeout),
              L.container.trigger('drag')),
            !1
          );
        })
        ...
        .mouseup(function () {
          return (
            (h = !1),
            clearTimeout(L.isMovingTimeout),
            (L.isMovingTimeout = setTimeout(function () {
              L.isMoving = !1;
            }, 100)),
            !1
          );
      ...
      jQuery(this.container).bind('touchmove', function (l) {
        var t,
          ...
          p,
          c,
          u = l.originalEvent.touches;
        if (1 === u.length) {
          if (1 === e) {
            if (s === u[0].pageX && r === u[0].pageY) return;
            (p = L.transX),
              (c = L.transY),
              (L.transX -= (s - u[0].pageX) / L.scale),
              (L.transY -= (r - u[0].pageY) / L.scale),
              L.applyTransform(),
              (p !== L.transX || c !== L.transY) && l.preventDefault(),
              (L.isMoving = !0),
              L.isMovingTimeout && clearTimeout(L.isMovingTimeout);
          }
  ...
  (JQVMap.prototype.select = function (l, t) {
    (l = l.toLowerCase()),
      ...
      this.isSelected(l) ||
        ...
        this.selectedColor &&
          t &&
          ((t.currentFillColor = this.selectedColor),
  ...
  (VectorCanvas.prototype.setSize = function (l, t) {
    if ('svg' === this.mode)
      ...
      for (
        var e = this.rootGroup.getElementsByTagName('shape'),
          a = 0,
          i = e.length;
        i > a;
        a++
